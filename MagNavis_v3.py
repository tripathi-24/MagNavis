...
class MagnavisApp(QWidget):
    ...
    def initUI(self):
        self.setWindowTitle("MagNavis_v3")
        ...
        self.anomaly_detection_method_descriptions = {
            ...
            "VolatilityShiftAD": (
                ...
                "Internally, it is implemented as a pipenet with transformer DoubleRollingAggregate.\n"
                "Useful for detecting positive shift of volatility, e.g., in seismic amplitude indicating start of earthquake.\n"
                "Parameters: c (float, e.g., 6.0), window (int, e.g., 30), side (default: 'positive').\n"
                "<a href='https://arundo-adtk.readthedocs-hosted.com/en/stable/examples.html#volatilityshiftad'>Learn more</a>"
                "</p>"
            ),
            "SeasonalAD": (
                "<p style='font-family: Arial; font-size: 12pt;'>"
                "SeasonalAD detects anomalous violations of seasonal pattern.\n"
                "Internally, it is implemented as a pipenet with transformer ClassicSeasonalDecomposition.\n"
                "Useful for detecting unusual traffic or events that violate expected seasonality, e.g., major holidays.\n"
                "Parameters: c (float, e.g., 3.0), side (default: 'both').\n"
                "<a href='https://arundo-adtk.readthedocs-hosted.com/en/stable/examples.html#seasonalad'>Learn more</a>"
                "</p>"
            ),
            "AutoregressionAD": (
                "<p style='font-family: Arial; font-size: 12pt;'>"
                "AutoregressionAD detects anomalous changes of autoregressive behavior in time series.\n"
                "Internally, it is implemented as a pipenet with transformers Retrospect and RegressionResidual.\n"
                "Useful for detecting violations of usual regressive behavior in time series, e.g., traffic history.\n"
                "Parameters: n_steps (int, e.g., 14), step_size (int, e.g., 24), c (float, e.g., 3.0).\n"
                "<a href='https://arundo-adtk.readthedocs-hosted.com/en/stable/examples.html#autoregressionad'>Learn more</a>"
                "</p>"
            )
        }
        ...
        self.tool_box.addItems([
            "Select Method", "Low Pass Filter", "High Pass Filter", "Band Pass Filter", "Time Lag Correction", 
            "Residual Mag Anomaly", "Analytic Signal Generation", "Outlier Removal", "Rolling Statistics", "Normalization/Standardization"
        ])
        ...
        self.filter_box3.addItem("Heatmap of Magnetic Values")
        self.filter_box3.addItem("Time Series Animation")
        self.filter_box3.addItem("Cluster Map (K-Means)")
        self.filter_box3.addItem("Correlation Matrix Heatmap")
        self.filter_box3.addItem("Histogram/Distribution Plot")
        self.filter_box3.addItem("Cross-Sectional Profile")
        self.filter_box3.addItem("PCA Plot")
...
# Open file dialog to select a CSV file
    ...
    def update_inputs(self):
        ...
        if method == "Threshold Detection":
            ...
            self.param1_label.setText("C Value (e.g., 6.0)")
            self.param2_label.setText("Window Size (e.g., 30)")
            self.param1_input.setPlaceholderText("e.g., 6.0")
            self.param2_input.setPlaceholderText("e.g., 30")
            self.param2_label.show()
            self.param2_input.show()

        elif method == "SeasonalAD":
            self.param1_label.setText("C Value (e.g., 3.0)")
            self.param1_input.setPlaceholderText("e.g., 3.0")
            self.param2_label.hide()
            self.param2_input.hide()

        elif method == "AutoregressionAD":
            self.param1_label.setText("n_steps (e.g., 14)")
            self.param1_input.setPlaceholderText("e.g., 14")
            self.param2_label.setText("step_size (e.g., 24)")
            self.param2_input.setPlaceholderText("e.g., 24")
            self.param2_label.show()
            self.param2_input.show()

        elif method == "CustomizedDetectorHD":
            self.param1_label.setText("column1 (e.g., Speed (kRPM))")
            self.param1_input.setPlaceholderText("e.g., Speed (kRPM)")
            self.param2_label.setText("column2 (e.g., Power (kW))")
            self.param2_input.setPlaceholderText("e.g., Power (kW)")
            self.param2_label.show()
            self.param2_input.show()
            # Add a third parameter for multiplier
            if not hasattr(self, 'param3_label'):
                self.param3_label = QLabel("multiplier (e.g., 1.2)")
                self.param3_input = QLineEdit()
                self.param3_input.setValidator(QDoubleValidator(0.0, 100000.0, 2))
                self.param3_input.setPlaceholderText("e.g., 1.2")
                self.layout().addWidget(self.param3_label)
                self.layout().addWidget(self.param3_input)
            self.param3_label.show()
            self.param3_input.show()
        else:
            if hasattr(self, 'param3_label'):
                self.param3_label.hide()
                self.param3_input.hide()
    ...
    def anomaly_detection(self):
        ...
        try:
            ...
            elif analysis_type2 == "VolatilityShiftAD" and param1 is not None and param2 is not None:
                data = validate_series(data)
                volatility_shift_ad = VolatilityShiftAD(c=param1, side="positive", window=int(param2))
                anomalies = volatility_shift_ad.fit_detect(data)
            elif analysis_type2 == "SeasonalAD" and param1 is not None:
                data = validate_series(data)
                seasonal_ad = SeasonalAD(c=param1, side="both")
                anomalies = seasonal_ad.fit_detect(data)
            elif analysis_type2 == "AutoregressionAD" and param1 is not None and param2 is not None:
                data = validate_series(data)
                # For AutoregressionAD, param1 = n_steps, param2 = step_size, c is fixed to 3.0 or can be added as a third param if needed
                autoregression_ad = AutoregressionAD(n_steps=int(param1), step_size=int(param2), c=3.0)
                anomalies = autoregression_ad.fit_detect(data)
            elif analysis_type2 == "CustomizedDetectorHD" and param1 is not None and param2 is not None and hasattr(self, 'param3_input') and self.param3_input.text():
                col1 = str(param1)
                col2 = str(param2)
                multiplier = float(self.param3_input.text())
                def myDetectionFunc(df):
                    return (df[col1] * multiplier > df[col2])
                numeric_data = self.filtered_data.select_dtypes(include=["number"])
                numeric_data.index = self.filtered_data.index
                # Ensure both columns are present
                if col1 not in numeric_data.columns or col2 not in numeric_data.columns:
                    QMessageBox.warning(self, "Error", f"Columns '{col1}' and/or '{col2}' not found in numeric columns.")
                    return
                customized_detector = CustomizedDetectorHD(detect_func=myDetectionFunc)
                anomalies = customized_detector.detect(numeric_data)
            else:
    ...
    def processing_tool(self):
        ...
        if analysis_type4 == "Low Pass Filter":
            ...
            self.analytic_signal_dialog = QDialog(self)
            self.analytic_signal_dialog.setWindowTitle("Analytic Signal Generation")
            self.analytic_signal_dialog.setGeometry(300, 300, 600, 400)
            self.channel_label = QLabel("Channel for Analytic Signal:")
            self.channel_dropdown = QComboBox()
            numeric_columns = self.filtered_data.select_dtypes(include=["number"]).columns
            if not self.filtered_data.empty and len(numeric_columns) > 0:
                self.channel_dropdown.addItems(numeric_columns)
            else:
                self.channel_dropdown.addItem("No numeric data available")
                self.channel_dropdown.setEnabled(False)
            # Checkboxes for components
            self.checkbox_envelope = QPushButton("Envelope", self.analytic_signal_dialog)
            self.checkbox_envelope.setCheckable(True)
            self.checkbox_envelope.setChecked(True)
            self.checkbox_phase = QPushButton("Phase", self.analytic_signal_dialog)
            self.checkbox_phase.setCheckable(True)
            self.checkbox_real = QPushButton("Real", self.analytic_signal_dialog)
            self.checkbox_real.setCheckable(True)
            self.checkbox_imag = QPushButton("Imaginary", self.analytic_signal_dialog)
            self.checkbox_imag.setCheckable(True)
            # Sliders for range
            self.range_label = QLabel("Plot Range:")
            ...
            max_idx = len(self.filtered_data) - 1
            self.slider_start.setMaximum(max_idx)
            self.slider_end.setMaximum(max_idx)
            self.slider_start.setValue(0)
            self.slider_end.setValue(max_idx)
            self.slider_start.setTickInterval(max(1, max_idx // 20))
            self.slider_end.setTickInterval(max(1, max_idx // 20))
            self.slider_start.setSingleStep(1)
            self.slider_end.setSingleStep(1)
            self.slider_start.valueChanged.connect(lambda: self.update_analytic_signal_plot(live=True))
            self.slider_end.valueChanged.connect(lambda: self.update_analytic_signal_plot(live=True))
            # Plot area
            self.figure = Figure(figsize=(8, 3))
            self.canvas = FigureCanvas(self.figure)
            # Export button
            self.export_button = QPushButton("Export as CSV")
            self.export_button.clicked.connect(self.export_analytic_signal_csv)
            # OK/Cancel
            self.ok_button = QPushButton("OK")
            self.cancel_button = QPushButton("Cancel")
            self.ok_button.clicked.connect(self.analytic_signal_dialog.accept)
            self.cancel_button.clicked.connect(self.analytic_signal_dialog.close)
            # Connect checkboxes
            self.checkbox_envelope.clicked.connect(lambda: self.update_analytic_signal_plot(live=True))
            self.checkbox_phase.clicked.connect(lambda: self.update_analytic_signal_plot(live=True))
            self.checkbox_real.clicked.connect(lambda: self.update_analytic_signal_plot(live=True))
            self.checkbox_imag.clicked.connect(lambda: self.update_analytic_signal_plot(live=True))
            self.channel_dropdown.currentIndexChanged.connect(lambda: self.update_analytic_signal_plot(live=True))
            # Layout
            layout = QVBoxLayout()
            layout.addWidget(self.channel_label)
            layout.addWidget(self.channel_dropdown)
            comp_layout = QHBoxLayout()
            comp_layout.addWidget(self.checkbox_envelope)
            comp_layout.addWidget(self.checkbox_phase)
            comp_layout.addWidget(self.checkbox_real)
            comp_layout.addWidget(self.checkbox_imag)
            layout.addLayout(comp_layout)
            layout.addWidget(self.range_label)
            range_layout = QHBoxLayout()
            range_layout.addWidget(QLabel("Start"))
            range_layout.addWidget(self.slider_start)
            range_layout.addWidget(QLabel("End"))
            range_layout.addWidget(self.slider_end)
            ...
            self.analytic_signal_dialog.setLayout(layout)
            # Compute analytic signal and plot initially
            self.update_analytic_signal_plot(live=False)
            self.analytic_signal_dialog.exec_()
        elif analysis_type4 == "High Pass Filter":
            self.hp_dialog = QDialog(self)
            self.hp_dialog.setWindowTitle("High Pass Filter")
            self.hp_dialog.setGeometry(300, 300, 400, 250)
            self.channel_label = QLabel("Channel to filter:")
            self.channel_dropdown = QComboBox()
            if not self.filtered_data.empty:
                self.channel_dropdown.addItems(self.filtered_data.columns)
            ...
            self.cutoff_input.setValidator(QDoubleValidator(0, 100000, 2))
            self.cutoff_input.setPlaceholderText("Nil (Default)")
            self.fs_label = QLabel("Sampling Frequency (Hz):")
            self.fs_input = QLineEdit()
            self.fs_input.setValidator(QDoubleValidator(0, 100000, 2))
            self.fs_input.setPlaceholderText("e.g., 1.0")
            self.ok_button = QPushButton("OK")
            self.cancel_button = QPushButton("Cancel")
            self.ok_button.clicked.connect(self.apply_high_pass_filter)
            self.cancel_button.clicked.connect(self.hp_dialog.close)
            layout = QVBoxLayout()
            layout.addWidget(self.channel_label)
            layout.addWidget(self.channel_dropdown)
            layout.addWidget(self.output_label)
            ...
            button_layout.addWidget(self.cancel_button)
            layout.addLayout(button_layout)
            self.channel_dropdown.currentIndexChanged.connect(lambda: self.output_name.setText(f"{self.channel_dropdown.currentText()}_HPF"))
            self.hp_dialog.setLayout(layout)
            self.hp_dialog.exec_()
        elif analysis_type4 == "Band Pass Filter":
            self.bp_dialog = QDialog(self)
            self.bp_dialog.setWindowTitle("Band Pass Filter")
            self.bp_dialog.setGeometry(300, 300, 400, 300)
            self.channel_label = QLabel("Channel to filter:")
            self.channel_dropdown = QComboBox()
            if not self.filtered_data.empty:
                self.channel_dropdown.addItems(self.filtered_data.columns)
            ...
            self.output_name.setPlaceholderText("Auto-generated name")
            self.output_name.setReadOnly(True)
            self.lowcut_label = QLabel("Low Cutoff Frequency (Hz):")
            self.lowcut_input = QLineEdit()
            self.lowcut_input.setValidator(QDoubleValidator(0, 100000, 2))
            self.lowcut_input.setPlaceholderText("Nil (Default)")
            self.highcut_label = QLabel("High Cutoff Frequency (Hz):")
            self.highcut_input = QLineEdit()
            self.highcut_input.setValidator(QDoubleValidator(0, 100000, 2))
            self.highcut_input.setPlaceholderText("Nil (Default)")
            self.fs_label = QLabel("Sampling Frequency (Hz):")
            self.fs_input = QLineEdit()
            self.fs_input.setValidator(QDoubleValidator(0, 100000, 2))
            self.fs_input.setPlaceholderText("e.g., 1.0")
            self.ok_button = QPushButton("OK")
            self.cancel_button = QPushButton("Cancel")
            self.ok_button.clicked.connect(self.apply_band_pass_filter)
            self.cancel_button.clicked.connect(self.bp_dialog.close)
            layout = QVBoxLayout()
            layout.addWidget(self.channel_label)
            layout.addWidget(self.channel_dropdown)
            layout.addWidget(self.output_label)
            ...
            layout.addWidget(self.lowcut_input)
            layout.addWidget(self.highcut_label)
            layout.addWidget(self.highcut_input)
            layout.addWidget(self.fs_label)
            layout.addWidget(self.fs_input)
            button_layout = QHBoxLayout()
            button_layout.addWidget(self.ok_button)
            button_layout.addWidget(self.cancel_button)
            layout.addLayout(button_layout)
            self.channel_dropdown.currentIndexChanged.connect(lambda: self.output_name.setText(f"{self.channel_dropdown.currentText()}_BPF"))
            self.bp_dialog.setLayout(layout)
            self.bp_dialog.exec_()
        elif analysis_type4 == "Outlier Removal":
            self.outlier_dialog = QDialog(self)
            self.outlier_dialog.setWindowTitle("Outlier Removal")
            self.outlier_dialog.setGeometry(300, 300, 400, 250)
            ...
            self.method_dropdown.addItems(["Z-Score", "IQR", "Percentile"])
            self.threshold_label = QLabel("Threshold:")
            self.threshold_input = QLineEdit()
            self.threshold_input.setValidator(QDoubleValidator(0, 100000, 2))
            self.threshold_input.setPlaceholderText("e.g., 3 for Z-Score, 1.5 for IQR")
            self.action_label = QLabel("Action:")
            self.action_dropdown = QComboBox()
            self.action_dropdown.addItems(["Remove", "Replace with NaN", "Replace with Mean", "Replace with Median"])
            self.ok_button = QPushButton("OK")
            self.cancel_button = QPushButton("Cancel")
            self.ok_button.clicked.connect(self.apply_outlier_removal)
            self.cancel_button.clicked.connect(self.outlier_dialog.close)
            layout = QVBoxLayout()
            layout.addWidget(self.channel_label)
            layout.addWidget(self.channel_dropdown)
            ...
            button_layout.addWidget(self.cancel_button)
            layout.addLayout(button_layout)
            self.outlier_dialog.setLayout(layout)
            self.outlier_dialog.exec_()
        elif analysis_type4 == "Rolling Statistics":
            self.rolling_dialog = QDialog(self)
            self.rolling_dialog.setWindowTitle("Rolling Statistics")
            self.rolling_dialog.setGeometry(300, 300, 400, 250)
            self.channel_label = QLabel("Channel:")
            self.channel_dropdown = QComboBox()
            if not self.filtered_data.empty:
                self.channel_dropdown.addItems(self.filtered_data.columns)
            ...
            self.window_input.setValidator(QDoubleValidator(1, 100000, 0))
            self.window_input.setPlaceholderText("e.g., 10")
            self.stat_label = QLabel("Statistic:")
            self.stat_dropdown = QComboBox()
            self.stat_dropdown.addItems(["Mean", "Std", "Min", "Max"])
            self.output_label = QLabel("Output Column:")
            self.output_name = QLineEdit()
            self.output_name.setPlaceholderText("Auto-generated name")
            self.ok_button = QPushButton("OK")
            self.cancel_button = QPushButton("Cancel")
            self.ok_button.clicked.connect(self.apply_rolling_statistic)
            self.cancel_button.clicked.connect(self.rolling_dialog.close)
            layout = QVBoxLayout()
            layout.addWidget(self.channel_label)
            layout.addWidget(self.channel_dropdown)
            ...
            layout.addWidget(self.stat_dropdown)
            layout.addWidget(self.output_label)
            layout.addWidget(self.output_name)
            button_layout = QHBoxLayout()
            button_layout.addWidget(self.ok_button)
            button_layout.addWidget(self.cancel_button)
            layout.addLayout(button_layout)
            self.rolling_dialog.setLayout(layout)
            self.rolling_dialog.exec_()
        elif analysis_type4 == "Normalization/Standardization":
            self.norm_dialog = QDialog(self)
            self.norm_dialog.setWindowTitle("Normalization/Standardization")
            self.norm_dialog.setGeometry(300, 300, 400, 250)
            self.channels_label = QLabel("Channels (select multiple):")
            self.channels_dropdown = QListWidget()
            self.channels_dropdown.setSelectionMode(QListWidget.MultiSelection)
            if not self.filtered_data.empty:
                self.channels_dropdown.addItems(self.filtered_data.columns)
            self.method_label = QLabel("Method:")
            self.method_dropdown = QComboBox()
            self.method_dropdown.addItems(["Min-Max (0-1)", "Z-Score (Standard)"])
            self.output_label = QLabel("Output Suffix:")
            self.output_name = QLineEdit()
            self.output_name.setPlaceholderText("e.g., _norm or _std")
            self.ok_button = QPushButton("OK")
            self.cancel_button = QPushButton("Cancel")
            self.ok_button.clicked.connect(self.apply_normalization)
            self.cancel_button.clicked.connect(self.norm_dialog.close)
            layout = QVBoxLayout()
            layout.addWidget(self.channels_label)
            layout.addWidget(self.channels_dropdown)
            layout.addWidget(self.method_label)
            layout.addWidget(self.method_dropdown)
            layout.addWidget(self.output_label)
            layout.addWidget(self.output_name)
            ...
            button_layout.addWidget(self.cancel_button)
            layout.addLayout(button_layout)
            self.norm_dialog.setLayout(layout)
            self.norm_dialog.exec_()

    def apply_high_pass_filter(self):
        from scipy.signal import butter, filtfilt
        selected_channel = self.channel_dropdown.currentText()
        output_column = self.output_name.text() or f"{selected_channel}_HPF"
        cutoff = self.cutoff_input.text()
        fs = self.fs_input.text()
        try:
            cutoff = float(cutoff)
            fs = float(fs)
            if fs <= 0:
            ...
            QMessageBox.warning(self.hp_dialog, "Invalid Input", "Please enter valid numbers for cutoff frequency and sampling frequency (fs > 0).")
            return
        normalized_cutoff = cutoff / (fs / 2)
        if not (0 < normalized_cutoff < 1):
            QMessageBox.warning(self.hp_dialog, "Invalid Cutoff", "Normalized cutoff must be between 0 and 1. Adjust cutoff or sampling frequency.")
            return
        if self.filtered_data.empty:
            QMessageBox.warning(self.hp_dialog, "No Data", "No data available to filter.")
            return
        b, a = butter(N=4, Wn=normalized_cutoff, btype='high', analog=False)
        self.filtered_data[output_column] = filtfilt(b, a, self.filtered_data[selected_channel].values)
        self.hp_dialog.close()
        self.update_table(self.filtered_data)
        plt.figure(figsize=(10, 5))
        plt.plot(self.filtered_data[selected_channel], label="Original", color='blue')
        plt.plot(self.filtered_data[output_column], label="High Pass Filtered", color='green')
        plt.xlabel("Index")
        plt.ylabel("Value")
        plt.title(f"High Pass Filtering: {selected_channel} -> {output_column}")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.show()
    ...
    def apply_band_pass_filter(self):
        from scipy.signal import butter, filtfilt
        selected_channel = self.channel_dropdown.currentText()
        output_column = self.output_name.text() or f"{selected_channel}_BPF"
        lowcut = self.lowcut_input.text()
        highcut = self.highcut_input.text()
        fs = self.fs_input.text()
        try:
            lowcut = float(lowcut)
            ...
            QMessageBox.warning(self.bp_dialog, "Invalid Input", "Please enter valid numbers for cutoff frequencies and sampling frequency (fs > 0).")
            return
        normalized_low = lowcut / (fs / 2)
        normalized_high = highcut / (fs / 2)
        if not (0 < normalized_low < 1) or not (0 < normalized_high < 1) or not (normalized_low < normalized_high):
            QMessageBox.warning(self.bp_dialog, "Invalid Cutoff", "Normalized cutoffs must be between 0 and 1 and low < high. Adjust cutoffs or sampling frequency.")
            return
        if self.filtered_data.empty:
            QMessageBox.warning(self.bp_dialog, "No Data", "No data available to filter.")
            return
        b, a = butter(N=4, Wn=[normalized_low, normalized_high], btype='band', analog=False)
        self.filtered_data[output_column] = filtfilt(b, a, self.filtered_data[selected_channel].values)
        self.bp_dialog.close()
        self.update_table(self.filtered_data)
        plt.figure(figsize=(10, 5))
        plt.plot(self.filtered_data[selected_channel], label="Original", color='blue')
        plt.plot(self.filtered_data[output_column], label="Band Pass Filtered", color='orange')
        plt.xlabel("Index")
        plt.ylabel("Value")
        plt.title(f"Band Pass Filtering: {selected_channel} -> {output_column}")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.show()
    ...
    def apply_outlier_removal(self):
        ...
        try:
            ...
            QMessageBox.warning(self.outlier_dialog, "Invalid Input", "Please enter a valid threshold.")
            return
        col = self.filtered_data[selected_channel]
        mask = None
        if method == "Z-Score":
            ...
            IQR = Q3 - Q1
            mask = (col < Q1 - threshold * IQR) | (col > Q3 + threshold * IQR)
        elif method == "Percentile":
            lower = col.quantile(threshold/100)
            upper = col.quantile(1 - threshold/100)
            mask = (col < lower) | (col > upper)
        col_new = col.copy()
        if action == "Remove":
            col_new = col_new[~mask]
        elif action == "Replace with NaN":
            col_new[mask] = np.nan
        elif action == "Replace with Mean":
            col_new[mask] = col.mean()
        elif action == "Replace with Median":
            col_new[mask] = col.median()
        self.filtered_data[selected_channel + "_outlier_cleaned"] = col_new
        self.outlier_dialog.close()
        self.update_table(self.filtered_data)
        plt.figure(figsize=(10, 5))
        plt.plot(col, label="Original", color='blue')
        plt.plot(self.filtered_data[selected_channel + "_outlier_cleaned"], label="Cleaned", color='red')
        plt.xlabel("Index")
        plt.ylabel("Value")
        plt.title(f"Outlier Removal: {selected_channel}")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.show()
    ...
    def apply_pca(self):
        pass  # This method is now unused; logic is handled inline in the dialog

    def apply_rolling_statistic(self):
        selected_channel = self.channel_dropdown.currentText()
        window = self.window_input.text()
        stat = self.stat_dropdown.currentText()
        output_column = self.output_name.text() or f"{selected_channel}_rolling_{stat.lower()}"
        try:
            window = int(window)
        except Exception:
            QMessageBox.warning(self.rolling_dialog, "Invalid Input", "Please enter a valid window size.")
            return
        if stat == "Mean":
            self.filtered_data[output_column] = self.filtered_data[selected_channel].rolling(window=window, center=True).mean()
        elif stat == "Std":
            self.filtered_data[output_column] = self.filtered_data[selected_channel].rolling(window=window, center=True).std()
        elif stat == "Min":
            self.filtered_data[output_column] = self.filtered_data[selected_channel].rolling(window=window, center=True).min()
        elif stat == "Max":
            self.filtered_data[output_column] = self.filtered_data[selected_channel].rolling(window=window, center=True).max()
        self.rolling_dialog.close()
        self.update_table(self.filtered_data)
        plt.figure(figsize=(10, 5))
        plt.plot(self.filtered_data[selected_channel], label="Original", color='blue')
        plt.plot(self.filtered_data[output_column], label=f"Rolling {stat}", color='magenta')
        plt.xlabel("Index")
        plt.ylabel("Value")
        plt.title(f"Rolling {stat}: {selected_channel}")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        plt.show()
    ...
    def apply_normalization(self):
        ...
        selected_items = self.channels_dropdown.selectedItems()
        if not selected_items:
            QMessageBox.warning(self.norm_dialog, "No Channels", "Please select at least one channel.")
            return
        selected_channels = [item.text() for item in selected_items]
        method = self.method_dropdown.currentText()
        output_suffix = self.output_name.text() or ("_norm" if method == "Min-Max (0-1)" else "_std")
        if method == "Min-Max (0-1)":
            scaler = MinMaxScaler()
        else:
            scaler = StandardScaler()
        X = self.filtered_data[selected_channels].values
        X_scaled = scaler.fit_transform(X)
        for i, col in enumerate(selected_channels):
            self.filtered_data[f"{col}{output_suffix}"] = X_scaled[:, i]
        self.norm_dialog.close()
        self.update_table(self.filtered_data)
        plt.figure(figsize=(10, 5))
        for i, col in enumerate(selected_channels):
            plt.plot(self.filtered_data[f"{col}{output_suffix}"], label=f"{col}{output_suffix}")
        plt.title(f"{method} Normalization/Standardization")
        plt.xlabel("Index")
        plt.ylabel("Value")
        plt.legend()
        plt.grid(True)
    ...
    def update_analytic_signal_plot(self, live=True):
        from scipy.signal import hilbert
        import numpy as np
        selected_channel = self.channel_dropdown.currentText()
        if selected_channel not in self.filtered_data.columns:
            return
        signal = self.filtered_data[selected_channel].values
        analytic_signal = hilbert(signal)
        self.filtered_data[f"{selected_channel}_Analytic_Real"] = np.real(analytic_signal)
        self.filtered_data[f"{selected_channel}_Analytic_Imag"] = np.imag(analytic_signal)
        self.filtered_data[f"{selected_channel}_Envelope"] = np.abs(analytic_signal)
        self.filtered_data[f"{selected_channel}_Phase"] = np.angle(analytic_signal)
        # Range
        ...
        if self.checkbox_envelope.isChecked():
            ax.plot(x, self.filtered_data[f"{selected_channel}_Envelope"][start:end], label="Envelope", color='purple')
            plotted = True
        if self.checkbox_phase.isChecked():
            ax.plot(x, self.filtered_data[f"{selected_channel}_Phase"][start:end], label="Phase", color='orange')
            plotted = True
        if self.checkbox_real.isChecked():
            ax.plot(x, self.filtered_data[f"{selected_channel}_Analytic_Real"][start:end], label="Real", color='blue')
            plotted = True
        if self.checkbox_imag.isChecked():
            ax.plot(x, self.filtered_data[f"{selected_channel}_Analytic_Imag"][start:end], label="Imaginary", color='green')
            plotted = True
        if plotted:
            ax.set_title(f"Analytic Signal Components: {selected_channel}")
            ax.set_xlabel("Index")
            ax.set_ylabel("Value")
            ax.legend()
            ax.grid(True)
        else:
            ax.text(0.5, 0.5, "Select at least one component to plot.", ha='center', va='center', transform=ax.transAxes)
        self.figure.tight_layout()
        self.canvas.draw()
        if not live:
            self.analytic_signal_dialog.repaint()

    def export_analytic_signal_csv(self):
        selected_channel = self.channel_dropdown.currentText()
        if selected_channel not in self.filtered_data.columns:
            QMessageBox.warning(self.analytic_signal_dialog, "No Data", "Selected channel not found in data.")
            return
        cols = [f"{selected_channel}_Analytic_Real", f"{selected_channel}_Analytic_Imag", f"{selected_channel}_Envelope", f"{selected_channel}_Phase"]
        export_df = self.filtered_data[[c for c in cols if c in self.filtered_data.columns]].copy()
        file_path, _ = QFileDialog.getSaveFileName(self, "Save Analytic Signal CSV", f"{selected_channel}_analytic_signal.csv", "CSV Files (*.csv)")
        if file_path:
            export_df.to_csv(file_path, index=False)
            QMessageBox.information(self.analytic_signal_dialog, "Export Successful", f"Analytic signal data exported to {file_path}")
    ...
    def update_inputs_vis(self):
        ...
        if method == "Anomaly Data Points Plot":
            ...
            self.param4_input.setPlaceholderText("e.g., 2")
            self.param5_input.setPlaceholderText("e.g., 0.5")
        elif method == "Heatmap of Magnetic Values":
            self.param3_input.setPlaceholderText("e.g., 0.5")
            self.param4_input.setPlaceholderText("e.g., 2")
            self.param5_input.setPlaceholderText("e.g., 0.5")
        elif method == "Time Series Animation":
            self.param3_input.setPlaceholderText("")
            self.param4_input.setPlaceholderText("")
            self.param5_input.setPlaceholderText("")
        elif method == "Cluster Map (K-Means)":
            self.param3_input.setPlaceholderText("Number of clusters (default 3)")
            self.param4_input.setPlaceholderText("")
            self.param5_input.setPlaceholderText("")
        elif method == "Correlation Matrix Heatmap":
            self.param3_input.setPlaceholderText("")
            self.param4_input.setPlaceholderText("")
            self.param5_input.setPlaceholderText("")
        elif method == "Histogram/Distribution Plot":
            self.param3_input.setPlaceholderText("")
            self.param4_input.setPlaceholderText("")
            self.param5_input.setPlaceholderText("")
        elif method == "Cross-Sectional Profile":
            self.param3_input.setPlaceholderText("Start Longitude")
            self.param4_input.setPlaceholderText("Start Latitude")
            self.param5_input.setPlaceholderText("End Longitude, End Latitude (comma separated)")
        elif method == "Interactive Map with Tooltips":
            self.param3_input.setPlaceholderText("")
            self.param4_input.setPlaceholderText("")
            self.param5_input.setPlaceholderText("")
        elif method == "PCA Plot":
            self.pca_dialog = QDialog(self)
            self.pca_dialog.setWindowTitle("PCA Plot")
            self.pca_dialog.setGeometry(300, 300, 400, 300)
            channels_label = QLabel("Channels (select multiple):")
            channels_dropdown = QListWidget()
            channels_dropdown.setSelectionMode(QListWidget.MultiSelection)
            if not self.filtered_data.empty:
                channels_dropdown.addItems(self.filtered_data.columns)
            n_components_label = QLabel("Number of Components:")
            n_components_input = QLineEdit()
            n_components_input.setValidator(QDoubleValidator(1, 10, 0))
            n_components_input.setPlaceholderText("e.g., 2")
            output_label = QLabel("Output Prefix:")
            output_name = QLineEdit()
            output_name.setPlaceholderText("PCA_")
            ok_button = QPushButton("OK")
            cancel_button = QPushButton("Cancel")
            def on_ok():
                selected_channels = [item.text() for item in channels_dropdown.selectedItems()]
                if not selected_channels:
                    QMessageBox.warning(self.pca_dialog, "No Channels Selected", "Please select at least one channel.")
                    return
                n_components = n_components_input.text()
                try:
                    n_components = int(n_components)
                except Exception:
                    QMessageBox.warning(self.pca_dialog, "Invalid Input", "Please enter a valid number of components.")
                    return
                if n_components < 1 or n_components > len(selected_channels):
                    QMessageBox.warning(self.pca_dialog, "Invalid Input", "Number of components must be at least 1 and at most the number of selected channels.")
                    return
                output_prefix = output_name.text() or "PCA_"
                try:
                    X = self.filtered_data[selected_channels]
                    # Ensure all columns are numeric
                    if not all(np.issubdtype(X[col].dtype, np.number) for col in X.columns):
                        QMessageBox.warning(self.pca_dialog, "Invalid Data", "All selected columns must be numeric.")
                        return
                    X = X.dropna()
                    if X.empty:
                        QMessageBox.warning(self.pca_dialog, "No Data", "Selected columns contain only NaN values after dropping missing data.")
                        return
                    from sklearn.decomposition import PCA
                    pca = PCA(n_components=n_components)
                    X_pca = pca.fit_transform(X)
                    for i in range(n_components):
                        self.filtered_data[f"{output_prefix}{i+1}"] = np.nan
                        self.filtered_data.loc[X.index, f"{output_prefix}{i+1}"] = X_pca[:, i]
                    self.pca_dialog.close()
                    self.update_table(self.filtered_data)
                    import matplotlib.pyplot as plt
                    plt.figure(figsize=(8, 6))
                    if n_components >= 2:
                        plt.scatter(self.filtered_data[f"{output_prefix}1"], self.filtered_data[f"{output_prefix}2"], alpha=0.7)
                        plt.xlabel(f"{output_prefix}1")
                        plt.ylabel(f"{output_prefix}2")
                        plt.title("PCA: First Two Components")
                    else:
                        plt.plot(self.filtered_data[f"{output_prefix}1"], label=f"{output_prefix}1")
                    ...
                    QMessageBox.critical(self.pca_dialog, "PCA Error", f"An error occurred during PCA computation:\n{str(e)}")
            ok_button.clicked.connect(on_ok)
            cancel_button.clicked.connect(self.pca_dialog.close)
            layout = QVBoxLayout()
            layout.addWidget(channels_label)
            layout.addWidget(channels_dropdown)
            layout.addWidget(n_components_label)
            layout.addWidget(n_components_input)
            ...
            layout.addLayout(button_layout)
            self.pca_dialog.setLayout(layout)
    ...
    def vis_method(self):
        ...
        self.submit_button.hide()
        self.description_box.hide()
        # Move error handling to the very top
        if self.filtered_data is None or self.filtered_data.empty:
        ...
        try:
            ...
            if analysis_type3 == "Anomaly Data Points Plot":
            ...
            elif analysis_type3 == "Heatmap of Magnetic Values":
                # Interpolate magnetic values onto a grid
                grid_z_mag = griddata(
                    (df_valid["Longitude"], df_valid["Latitude"]),
                    df_valid["Magnetic_Values"],
                    (grid_x_mesh, grid_y_mesh),
                    method="cubic"
                )
                fig, ax = plt.subplots(figsize=(12, 8))
                ...
                cbar.set_label("Magnetic Value (nT)")
                ax.set_title("Heatmap of Magnetic Values")
                ax.set_xlabel("Longitude")
                ax.set_ylabel("Latitude")
                plt.grid(True, linestyle='--', alpha=0.2)
                plt.tight_layout()
                plt.show()
            elif analysis_type3 == "Time Series Animation":
                # Animate the spatial distribution of magnetic values over time
                if "Time" not in self.filtered_data.columns:
                    QMessageBox.warning(self, "No Time Data", "No 'Time' column found in the data for animation.")
                    return
                import matplotlib.animation as animation
                df_valid_anim = self.filtered_data.dropna(subset=["Latitude", "Longitude", "Magnetic_Values", "Time"])
                unique_times = np.sort(df_valid_anim["Time"].unique())
                if len(unique_times) < 2:
                    QMessageBox.warning(self, "Insufficient Data", "Not enough time points for animation.")
                    return
                fig, ax = plt.subplots(figsize=(10, 7))
                scat = ax.scatter([], [], c=[], cmap='coolwarm', s=20, vmin=df_valid_anim["Magnetic_Values"].min(), vmax=df_valid_anim["Magnetic_Values"].max())
                ax.set_xlim(df_valid_anim["Longitude"].min(), df_valid_anim["Longitude"].max())
                ax.set_ylim(df_valid_anim["Latitude"].min(), df_valid_anim["Latitude"].max())
                ax.set_xlabel("Longitude")
                ax.set_ylabel("Latitude")
                ax.set_title("Magnetic Values Over Time")
                cbar = plt.colorbar(scat, ax=ax, orientation='vertical', shrink=0.8, pad=0.02)
                cbar.set_label("Magnetic Value (nT)")
                def update(frame):
                    t = unique_times[frame]
                    df_t = df_valid_anim[df_valid_anim["Time"] == t]
                    scat.set_offsets(np.c_[df_t["Longitude"], df_t["Latitude"]])
                    scat.set_array(df_t["Magnetic_Values"].values)
                    ax.set_title(f"Magnetic Values at Time: {t}")
                    return scat,
                ani = animation.FuncAnimation(fig, update, frames=len(unique_times), interval=500, blit=True, repeat=True)
                plt.show()
            elif analysis_type3 == "Cluster Map (K-Means)":
                # Cluster the data points using K-Means and plot them on a map
                from sklearn.cluster import KMeans
                df_valid_cluster = self.filtered_data.dropna(subset=["Latitude", "Longitude"])
                if df_valid_cluster.empty:
                    QMessageBox.warning(self, "No Data", "No valid data for clustering.")
                    return
                try:
                    n_clusters = int(self.param3_input.text()) if self.param3_input.text() else 3
                except Exception:
                    n_clusters = 3
                coords = df_valid_cluster[["Longitude", "Latitude"]].values
                kmeans = KMeans(n_clusters=n_clusters, n_init=10)
                labels = kmeans.fit_predict(coords)
                df_valid_cluster["Cluster"] = labels
                fig, ax = plt.subplots(figsize=(10, 7))
                scatter = ax.scatter(df_valid_cluster["Longitude"], df_valid_cluster["Latitude"], c=labels, cmap="tab10", s=20)
                ...
                plt.tight_layout()
                plt.show()
            elif analysis_type3 == "Correlation Matrix Heatmap":
                # Plot a heatmap of the correlation coefficients between all numeric columns
                numeric_cols = self.filtered_data.select_dtypes(include=[np.number])
                if numeric_cols.shape[1] < 2:
                    QMessageBox.warning(self, "Insufficient Data", "Not enough numeric columns for correlation heatmap.")
                    return
                corr = numeric_cols.corr()
                import seaborn as sns
                fig, ax = plt.subplots(figsize=(8, 6))
                sns.heatmap(corr, annot=True, fmt=".2f", cmap="coolwarm", ax=ax, square=True, cbar_kws={"shrink": 0.8})
                ax.set_title("Correlation Matrix Heatmap")
                plt.tight_layout()
                plt.show()
            elif analysis_type3 == "Histogram/Distribution Plot":
                # Only open dialog if data is loaded and valid
                if self.filtered_data is None or self.filtered_data.empty:
                    QMessageBox.warning(self, "No Data", "No data loaded or selected. Please open a CSV file and select columns before visualisation.")
                    return
                ...
                hist_dialog.setWindowTitle("Select Column for Histogram")
                layout = QVBoxLayout()
                label = QLabel("Select a numeric column:")
                combo = QComboBox()
                ...
                def plot_hist():
                    ...
                    data = self.filtered_data[col].dropna()
                    fig, ax = plt.subplots(figsize=(8, 5))
                    ax.hist(data, bins=30, color='skyblue', edgecolor='black', alpha=0.8)
                    ...
                    plt.show()
                    hist_dialog.close()
                ok_btn.clicked.connect(plot_hist)
                hist_dialog.exec_()
            elif analysis_type3 == "Cross-Sectional Profile":
                # Only open dialog if data is loaded and valid
                if self.filtered_data is None or self.filtered_data.empty:
                    QMessageBox.warning(self, "No Data", "No data loaded or selected. Please open a CSV file and select columns before visualisation.")
                    return
                ...
                profile_dialog.setWindowTitle("Cross-Sectional Profile Coordinates")
                layout = QVBoxLayout()
                label1 = QLabel("Start Longitude:")
                start_lon_input = QLineEdit()
                label2 = QLabel("Start Latitude:")
                start_lat_input = QLineEdit()
                label3 = QLabel("End Longitude:")
                end_lon_input = QLineEdit()
                label4 = QLabel("End Latitude:")
                end_lat_input = QLineEdit()
                ok_btn = QPushButton("OK")
                layout.addWidget(label1)
                layout.addWidget(start_lon_input)
                layout.addWidget(label2)
                layout.addWidget(start_lat_input)
                layout.addWidget(label3)
                layout.addWidget(end_lon_input)
                layout.addWidget(label4)
                layout.addWidget(end_lat_input)
                layout.addWidget(ok_btn)
                profile_dialog.setLayout(layout)
                def plot_profile():
                    try:
                        start_lon = float(start_lon_input.text())
                        start_lat = float(start_lat_input.text())
                        end_lon = float(end_lon_input.text())
                        end_lat = float(end_lat_input.text())
                    except Exception:
                        QMessageBox.warning(profile_dialog, "Invalid Input", "Please enter valid coordinates.")
                        return
                    # Interpolate values along the line
                    n_points = 200
                    lons = np.linspace(start_lon, end_lon, n_points)
                    lats = np.linspace(start_lat, end_lat, n_points)
                    points = np.column_stack((self.filtered_data["Longitude"], self.filtered_data["Latitude"]))
                    values = self.filtered_data["Magnetic_Values"]
                    from scipy.interpolate import griddata
                    profile_values = griddata(points, values, (lons, lats), method='linear')
                    dist = np.sqrt((lons - start_lon)**2 + (lats - start_lat)**2)
                    fig, ax = plt.subplots(figsize=(10, 5))
                    ax.plot(dist, profile_values, color='purple')
                    ax.set_title("Cross-Sectional Profile of Magnetic Values")
                    ax.set_xlabel("Distance (degrees)")
                    ax.set_ylabel("Magnetic Value (nT)")
                    plt.tight_layout()
                    plt.show()
                    profile_dialog.close()
                ok_btn.clicked.connect(plot_profile)
                profile_dialog.exec_()
            elif analysis_type3 == "Interactive Map with Tooltips":
                # Create an interactive map with tooltips using folium
                import folium
                import webbrowser
                df_valid_map = self.filtered_data.dropna(subset=["Latitude", "Longitude"])
                if df_valid_map.empty:
                    QMessageBox.warning(self, "No Data", "No valid data for interactive map.")
                    return
                center_lat = df_valid_map["Latitude"].mean()
                center_lon = df_valid_map["Longitude"].mean()
                m = folium.Map(location=[center_lat, center_lon], zoom_start=8)
                for _, row in df_valid_map.iterrows():
                    tooltip = f"Magnetic Value: {row.get('Magnetic_Values', 'N/A')}<br>"
                    tooltip += f"Time: {row.get('Time', 'N/A')}<br>"
                    tooltip += f"Residual Anomaly: {row.get('Residual Anomaly (nT)', 'N/A')}"
                    folium.CircleMarker(
